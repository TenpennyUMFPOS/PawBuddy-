...
import 'package:project_integration_pb/core/constants/firebase_constants.dart';
...
class CommunityRepository {
  ...
  FutureVoid createCommunity(Community community) async {
    try {
      ...
      if (communityDoc.exists) {
        throw 'A Community with the same name already exists!';
      }
  ...
  Stream<List<Post>> getCommunityPosts(String name) {
    return _posts
        .where('communityName', isEqualTo: name).orderBy('createdAt', descending: true).snapshots()
        .map(
          (event) => event.docs
  ...
  FutureVoid upgradeCommunity(String communityName) async {
    try {
      return right(_communities.doc(communityName).update({
        'isPro': true,
      }));
